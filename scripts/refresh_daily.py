...
# ── config ────────────────────────────────────────────────────────────────────
BATCH_SIZE  = 60         # no API rate limit — fetch all remaining teams in one run
STATS_TTL   = 7 * 86400  # re-fetch after 7 days
DELAY       = 1.5        # seconds between team fetches
STOP_DATE   = date(2026, 7, 20) # run through the World Cup final (July 19)
...
def fetch_teams(teams: list[str], cache: dict) -> tuple[list, list]:
    from src.scrapers.github_stats_client import get_team_stats
    ...
    for i, team in enumerate(teams):
        ...
        try:
            ...
            if stats:
                ...
                ) if has_ext else "(goals only — GitHub CSV source)"
                log.info(f"    ✓ GF={stats['GF_AVG']}  GA={stats['GA_AVG']}  {ext}")
                ok.append(team)
            else:
                log.warning(f"    — no data returned (team not found in dataset)")
                failed.append(team)
